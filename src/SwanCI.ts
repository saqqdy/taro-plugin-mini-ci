...
export default class SwanCI extends BaseCI {
	...
	async open() {
		...
		const devToolsInstallPath =
			this.pluginOpts.swan!.devToolsInstallPath ||
	...
	async preview() {
		...
		shell.exec(
			`${this.swanBin} preview --project-path ${this.projectPath} --token ${
				this.pluginOpts.swan!.token
			} --min-swan-version ${this.pluginOpts.swan!.minSwanVersion || '3.350.6'} --json`,
	...
	async upload() {
		...
		shell.exec(
			`${this.swanBin} upload --project-path ${this.projectPath} --token ${
				this.pluginOpts.swan!.token
			} --release-version ${this.version} --min-swan-version ${
				this.pluginOpts.swan!.minSwanVersion || '3.350.6'
			} --desc ${this.desc} --json`,
			async (_code, stdout, stderr) => {
				if (!stderr) {
