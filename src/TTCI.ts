...
export default class TTCI extends BaseCI {
	...
	async preview() {
		...
		try {
			...
			printLog(
				...
				`预览二维码已生成，存储在:"${previewQrcodePath}",二维码内容是：${qrContent},过期时间：${new Date(
					previewResult.expireTime * 1000
				).toLocaleString()}`
			)
	...
	async upload() {
		...
		try {
			...
			printLog(
				processTypeEnum.REMIND,
				`体验版二维码已生成，存储在:"${uploadQrcodePath}",二维码内容是："${qrContent}", 过期时间：${new Date(
					uploadResult.expireTime * 1000
				).toLocaleString()}`
			)
