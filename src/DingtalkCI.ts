...
export default class DingtalkCI extends BaseCI {
	...
	init(): void {
		...
		try {
			...
			printLog(
				...
				chalk.red('请安装依赖：dingtalk-miniapp-opensdk , 该依赖用于CI预览、上传钉钉小程序')
			)
			process.exit(1)
		}
	...
	async open() {
		...
		try {
			...
			printLog(
				processTypeEnum.ERROR,
				chalk.red('请安装依赖：minidev ,该依赖用于编译后自动打开小程序开发者工具')
			)
			process.exit(1)
		}
		try {
	...
	async preview() {
		...
		try {
			...
			printLog(
				...
				`预览版二维码已生成，存储在:"${previewQrcodePath}",二维码内容是："${previewUrl}"`
			)
	...
	async upload() {
		...
		try {
			...
			console.log(
				chalk.green(`版本 ${result.packageVersion} 上传成功 ${new Date().toLocaleString()}`)
			)
